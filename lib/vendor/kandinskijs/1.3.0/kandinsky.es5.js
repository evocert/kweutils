/* 
skullquake - in progress ... Tue Jan  5 03:59:32 PST 2021
*/
(function (global, factory) {
  ...
  var wrapNorm = wrapValue(0, 1);

  // clamp :: Number -> Number -> Number
  var clamp = curry((min, max, n) => {
    if (n < min) return min;
    if (n > max) return max;
    return n;
  });
  ...
  var padHex = hexStr => hexStr.length === 1 ? `0${hexStr}` : hexStr;
  ...
  var rgb2hex = rgb => rgb.reduce((hex, c) => hex + padHex(c.toString(16)), '#');
  ...
  var darkenHex = curry((amount, hex) => pipe(
    hex2rgb,
    darkenRgb(amount),
    rgb2hex
  )(hex));
  ...
  var lerp3 = curry((t, [a1, b1, c1], [a2, b2, c2]) => [
    a1 + t * (a2 - a1),
    b1 + t * (b2 - b1),
    c1 + t * (c2 - c1),
  ]);
  ...
  var linearGradient = curry((n, c1, c2) => {
    var d = (n-1 !== 0) ? n-1 : 1;
  ...
  var rGradient = curry((ease, n, c1, c2) => gradient(ease, n, c1, c2).map(color => color.map(Math.round)));

  // rLinearGradient :: Int -> [Number, Number, Number] -> [Number, Number, Number] -> [[Int, Int, Int]]
  var rLinearGradient = curry((n, c1, c2) => linearGradient(n, c1, c2).map(color => color.map(Math.round)));

  // multiGradient :: Int -> [[Number, Number, Number]] -> [[Int, Int, Int]]
  var rMultiGradient = curry((n, colors) => multiGradient(n, colors).map(color => color.map(Math.round)));

  // complimentHsl :: Int -> [Number, Number, Number] -> [Number, Number, Number]
  var complimentHsl = curry((n, [h, s, l]) => Array.from(Array(n), (_, i) => [
    wrapNorm(h - (i/n)),
    s,
    l
  ]));
  ...
  var complimentRgb = curry((n, rgb) => pipe(
    rgb2hsl,
    complimentHsl(n),
  ...
  var complimentHex = curry((n, hex) => pipe(
    hex2hsl,
    complimentHsl(n),
  ...
  var rgb2css = curry((alpha, rgb) => {
    var a = typeof alpha === 'number' ? clampNorm(alpha) : 1;
    var [r, g, b] = rgb.map(pipe(Math.round, clamp(0, 255)));
    return `rgba(${r}, ${g}, ${b}, ${a})`;
  });

  // hsl2css :: Number -> [Number, Number, Number] -> String
  var hsl2css = curry((alpha, [h, s, l]) => {
    var a = typeof alpha === 'number' ? clampNorm(alpha) : 1;
