...
//>>excludeStart('excludeBabel', pragmas.excludeBabel)
	...
	*/
	{//skullquake todo:propper switching via options/config:todo
            fetchText = function (path, callback) {
			console.log('es6:loading '+path);
			var source=fsutils.File2String(path);
			callback(source);
			/*
			require(["text!"+path],function(t){
				callback(value);
			});
			*/
            };
...
//>>excludeStart('excludeBabel', pragmas.excludeBabel)
		...
		options.presets=["es2015"];//skullquake
