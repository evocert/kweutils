...
define("dojo/on", ["./has!dom-addeventlistener?:./aspect", "./_base/kernel", "./sniff"], function (y, z, f) {
	if(typeof(GOJA)!='undefined'){
		function A(a, c, b, d, e) {
			if ((d = c.match(/(.*):(.*)/))) return (c = d[2]), (d = d[1]), g.selector(d, c).call(e, a, b);
			f("touch") && (B.test(c) && (b = n(b)), f("event-orientationchange") || "orientationchange" != c || ((c = "resize"), (a = window), (b = n(b))));
			p && (b = p(b));
			if (a.addEventListener) {
				var k = c in q,
					m = k ? q[c] : c;
				a.addEventListener(m, b, k);
				return {
					remove: function () {
						a.removeEventListener(m, b, k);
					},
				};
			}
			if (r && a.attachEvent) return r(a, "on" + c, b);
		...
		f("touch") &&
			f.add("touch-can-modify-event-delegate", function () {
				var a = function () {};
		...
		g.pausable = function (a, c, b, d) {
			...
			a = g(
				...
				c,
				function () {
					if (!e) return b.apply(this, arguments);
				},
				d
			);
			a.pause = function () {
		...
		g.parse = function (a, c, b, d, e, k) {
			var f;
			if (c.call) return c.call(k, a, b);
			c instanceof Array ? (f = c) : -1 < c.indexOf(",") && (f = c.split(/\s*,\s*/));
			if (f) {
				var h = [];
				c = 0;
				for (var u; (u = f[c++]); ) h.push(g.parse(a, u, b, d, e, k));
				h.remove = function () {
		...
		g.matches = function (a, c, b, d, e) {
			...
			for (; !e.matches(a, c, b); ) if (a == b || !1 === d || !(a = a.parentNode) || 1 != a.nodeType) return !1;
			return a;
		};
		g.selector = function (a, c, b) {
			return function (d, e) {
				...
				var m = "function" == typeof a ? { matches: a } : this,
					h = c.bubble;
				return h
					? g(d, h(f), e)
					: g(d, c, function (a) {
		...
		if (!f("event-stopimmediatepropagation"))
			var G = function () {
				...
				p = function (a) {
					return function (c) {
						if (!c.immediatelyStopped) return (c.stopImmediatePropagation = G), a.apply(this, arguments);
					};
				};
			...
			g._fixEvent = function (a, c) {
				...
				if (!a.target)
					switch (
						...
						a.stopPropagation || ((a.stopPropagation = H), (a.preventDefault = I)),
						a.type)
					) {
						case "keypress":
				...
				n = function (a) {
					return function (c) {
						...
						if (!b) {
							...
							if (c.type) {
								if (f("touch-can-modify-event-delegate")) (w.prototype = c), (b = new w());
								else {
									var b = {},
										e;
									for (e in c) b[e] = c[e];
							...
							c.corrected = b;
							if ("resize" == d) {
								if (x == window.orientation) return null;
								x = window.orientation;
