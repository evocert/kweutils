...
define("dojo/_base/kernel", ["../has", "./config", "require", "module"], function (d, h, k, b) {
    ...
    var e = {},
        m = {},
        a = { config: h, global: g, dijit: e, dojox: m },
        e = { dojo: ["dojo", a], dijit: ["dijit", e], dojox: ["dojox", m] };
    b = k.map && k.map[b.id.match(/[^\/]+/)[0]];
    for (c in b) e[c] ? (e[c][0] = b[c]) : (e[c] = [b[c], {}]);
    for (c in e) (b = e[c]), (b[1]._scopeName = b[0]), h.noGlobals || (g[b[0]] = b[1]);
    a.scopeMap = e;
    a.baseUrl = a.config.baseUrl = k.baseUrl;
    a.isAsync = k.async;
    a.locale = h.locale;
    g = "$Rev: 594ed6f $".match(/[0-9a-f]{7,}/);
    a.version = {
        major: 1,
        minor: 12,
        patch: 2,
    ...
    for (g = 0; (l = b[g++]); )
        console[l]
            ? d("console-as-object") && (console[l] = Function.prototype.bind.call(console[l], console))
            : (function () {
		/*
                  var f = l + "";
                  console[f] =
    ...
    if (h.modulePaths) {
        ...
        d = {};
        for (c in h.modulePaths) d[c.replace(/\./g, "/")] = h.modulePaths[c];
        k({ paths: d });
    }
