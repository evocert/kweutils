									...
									if(optionsarg.node!=null){
										...
										iframe=$("<iframe>")
											...
											.on('load',function(){
												...
												`));
												iframe.contents().find('body').append($("<script/>").text(`
													window.console=window.parent.console;
												`));
												/*
												iframe[0].contentWindow.addEventListener("log", function (value) {
													//console.log.apply(null, value);
													iframe[0].contentWindow.parent.console.log.apply(null,value);
													iframe[0].contentWindow.addEventListener("log",function(){alert('hi');})
												});
												*/
											})
										;
