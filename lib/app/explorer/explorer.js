require([
	...
	try{
				...
				function prvmkwin(optionsarg){
					...
					if(optionsarg.fullscreen||mobileCheck()){
						...
						if(optionsarg.node!=null){
							...
							iframe=$("<iframe>")
								.attr("src",
									//optionsarg.iframesrc.startsWith("./")||optionsarg.iframesrc.startsWith("../")?
									//[cwd,optionsarg.iframesrc].join('/')://todo cwd stuff to explorer instance
								...
								.on('load',function(){
									//iframe.contents().find('body').append($("<link/>").attr("rel","stylesheet").attr("href","http://localhost/explorer/css/style.css"))
									loadingmsg.remove();
						...
						jsPanel.create({
							...
							callback:$.proxy(function(panel){
								if(optionsarg.node!=null){
									...
									iframe=$("<iframe>")
										...
										.attr("width",content.width())//-20)
										.attr("height",content.height())//-20)
										//.attr("scrolling",'no')
										.css({border:"none"})
										.css({background:"#222222"})
										.on('load',function(){
							...
							position:{
								...
								//at:'center-center',
								//of:'body',
								offsetX:optionsarg.x,//'0',
								offsetY:optionsarg.y,//'0',
								//autoposition:'down'
							},
							...
							resizeit:optionsarg.iframesrc==null?optionsarg.resizeit:{
								start:$.proxy(function(panel,size){
									panel.iframe.attr("width",size.width);//-20);
									panel.iframe.attr("height",size.height);//-40);
								},this),
				...
				function prvls(path){
					return new Promise(function(resolve,reject){
						...
						$.ajax(
							{
								type:"POST",
								url:"/explorer/goja.html?test=tests/svc_fs",
								...
								success:$.proxy(function(data){
									data.push({
										...
										"path": cwd,
										"type": "Dir",
										"icon": "/explorer/img/explorer/back.svg",
										"classlist": ['back']
									});
									data.forEach(function(entry){
										entry.relpath=pathutils.normalizepath(['explorer','.',path,entry.name].join('/'));	
									});
									cwd=path;
									//data manipulation entry
								...
								always:$.proxy(function(){//not triggering
									resolve();
								},this)
							}
						);
		...
		View=(function(options){
			options=options==null?{}:options;
			options.node=options.node==null?$('#explorer').length>0?$('#explorer'):$('body'):options.node;
			var node=options.node;
			container=$('<div/>').addClass("explorer");
  ...
  margin:auto;
			...
			var tpl_icon=_.template(`
				<div class="icon explorertooltip <%= typeof(classlist)=='undefined'?'':classlist.join(' ') %>" <% if(name.startsWith('.')&&name!='..'){%>style="display:none!important;"<%}%>>
					...
					<%}else if(typeof(favicon)!='undefined') { %>
						<img class="iconimage" src=<%= ['.',relpath,favicon].join('/') %>></img>
					<%}else if(type=="Dir") { %>
						<img class="iconimage" src="explorer/img/explorer/folder-black.svg"></img>
					<%}else if(type=="File") { %>
						<% if(name.endsWith(".html")){ %>
							<img class="iconimage" src="explorer/img/explorer/text-html.svg"></img>
						<% }else{ %>
							<img class="iconimage" src="explorer/img/explorer/text-x-generic.svg"></img>
						<% } %>
					<%}else{%>
						<img class="iconimage"  src="explorer/img/explorer/application-document.svg"></img>
					<%}%>
					<div class="iconname"><%= /*name.length>8?[name.substring(0,4),'...'].join(' '):name*/name %></div>
				</div>
			`);
			var prvrender=function(topics,data){
				...
				datasorted.forEach($.proxy(function(entry){
					if(entry.name.indexOf('..')!=0&&entry.name.indexOf('.')==0)return;//hide dotfiles
					...
					if(entry.relpath!=null&&entry.relpath!=''){
						/*
						icon.append($('<span/>').addClass('explorertooltiptext'));
						icon.hover(function(el){
							$(this).find('.explorertooltiptext').text($(this).data('relpath'));
						});
						*/
					}
						...
						$.ajax(
							{
								...
								success:$.proxy(function(data){
									try{
										//eval.call({"icon":icon,"entry":entry},data);
										eval(data);
									}catch(e){
										console.error(e)
									}
							...
							success:$.proxy(function(data){
								jsPanel.create({
									...
									callback:$.proxy(function(panel){
										...
										//$(panel.content).css({"padding":"unset","margin":"unset","width":"100%"});
									}),
					...
					$.ajax(
						{
							...
							success:$.proxy(function(data){
								jsPanel.create({
									...
									callback:$.proxy(function(panel){
										var htmlwrapper=$("<div/>");
										htmlwrapper.attr("id","htmlwrapper");
										htmlwrapper.append(data);
										$(panel.content).append(htmlwrapper);
									}),
									panelSize:[400,300].join(" "),
									headerControls:{
										size:"xs"
		...
		//--------------------------------------------------------------------------------
		{//instance
			var mod=Mod.getInstance({root:"."});
			var ctl=Ctl(mod);
			var view=View({ctl:ctl,mod:mod});
			ctl.ls();
		}
		//--------------------------------------------------------------------------------
	}catch(e){
