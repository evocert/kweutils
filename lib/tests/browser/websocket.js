...
window._=_;
	...
	function loop(){
		if(loopon){
			...
			loopidx++;
			status.text(["idx:",loopidx,"cps:",1000*loopidx/(new Date()-loopstart)].join(" "));
			socket.send(
				valproc
			);
		}else{
...
`
	...
	`));
	var info=$("<div/>").attr({}).addClass("info").text("ctrl-enter: execute, alt-l: toggle looped execute, alt-m: toggle sound");
