//OAuth 1.0a signature generator for node and the browser
//https://github.com/bettiolo/oauth-signature-js
//example: http://bettiolo.github.io/oauth-reference-page/
//note: error about url...
define([
	"module",
	"oauthsignature",
],function(
	module,
	oauthSignature
){
	console.log([module.id,'start'].join(':'));
	console.log(oauthSignature);
	{//basic usage
		var httpMethod = 'GET';
		var url = 'http://photos.example.net/photos';
		var parameters = {
			oauth_consumer_key : 'dpf43f3p2l4k3l03',
			oauth_token : 'nnch734d00sl2jdk',
			oauth_nonce : 'kllo9940pd9333jh',
			oauth_timestamp : '1191242096',
			oauth_signature_method : 'HMAC-SHA1',
			oauth_version : '1.0',
			file : 'vacation.jpg',
			size : 'original'
		};
		var consumerSecret = 'kd94hf93k423kf44';
		var tokenSecret = 'pfkkdhi9sl3r4s00';
		// generates a RFC 3986 encoded, BASE64 encoded HMAC-SHA1 hash
		var encodedSignature = oauthSignature.generate(httpMethod, url, parameters, consumerSecret, tokenSecret);//error: Uncaught ReferenceError: url is not defined
		// generates a BASE64 encode HMAC-SHA1 hash
		var signature = oauthSignature.generate(httpMethod, url, parameters, consumerSecret, tokenSecret,{ encodeSignature: false});
	}
});
